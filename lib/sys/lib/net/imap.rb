...
module Sys::Lib::Net::Imap
  ...
  def self.connect
    ...
    imap = nil
    begin
      raise 'no_address' unless Core.current_user.email =~ /^(.*)@/
#      username = $1
      username = Core.current_user.email
      password = Core.current_user.password
#      password = Core.current_user.imap_password
      timeout(10) do
      ...
      raise "IMAP: 認証に失敗しました。システム管理者に問い合わせてください。" if e.message =~ /Authentication failed./
