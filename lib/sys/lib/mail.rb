...
module Sys::Lib::Mail
  ...
  def received_date(format = '%Y-%m-%d %H:%M', nullif = nil)
    if @mail.header[:received].blank?
      return nullif
    else
      received = @mail.header[:received].to_s.split(";")
      return received[1].to_time.strftime(format)
    end
  end
