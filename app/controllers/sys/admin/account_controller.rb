...
class Sys::Admin::AccountController < Sys::Controller::Admin::Base
  ...
  def login
    skip_layout
    return redirect_to(make_login_url) if make_login_url.present?
    #admin_uri = '/_admin'
...
#    end
  ...
  def sso
    ...
    params[:to] ||= 'gw'
    raise 'SSOの設定がありません。' unless config = Enisys.config.sso_settings[params[:to].to_sym]
